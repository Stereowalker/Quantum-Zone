package quantumzone.stereowalker.tileentity;
...
import quantumzone.stereowalker.capabilities.Worker;
import quantumzone.stereowalker.init.Blk;
import quantumzone.stereowalker.util.Utils;
...
public class TileEntityMiner extends TileEntity implements ITickable, ICapabilityProvider {
	...
	public void update() {
		if (this.world != null) { // Makes sure we have a world. RENAMED IN
			...
			if (!this.world.isRemote && this.world.isBlockPowered(pos)) { // Calls it server side and checks if our block is powered
				...
				this.world.setBlockState(pos, currentState.withProperty(BlockMiner.ACTIVATED, Boolean.valueOf(true))); // Updates it if it is powered
				...
				if (this.cooldown == 0) { // Only runs when the cooldown is 0 (i.e every 50 or 100 ticks (2.5 or 5 seconds))
					...
					EnumFacing facing = (EnumFacing) currentState.getValue(BlockMiner.FACING); // Gets which way our block is facing
				...
				if (!this.world.isAirBlock(pos) && this.world.getBlockState(pos).getBlock() == Blk.MINER) { // The block is not air and it is a block breaker
					if (this.world.getBlockState(pos).getValue(BlockMiner.ACTIVATED)) { // Checks if it is activated
						IBlockState currentState = this.world.getBlockState(pos);
						this.world.setBlockState(pos,
								currentState.withProperty(BlockMiner.ACTIVATED, Boolean.valueOf(false))); // Makes it not activated
					}
				}
			}
		}
	...
	public void breakBlock(EnumFacing facing) {
		...
		if (!block.isAir(state, this.world, newPos) && block.getBlockHardness(state, this.world, newPos) >= 0
			...
			EntityPlayer player = new EntityPlayer(world, new GameProfile(null, "Miner")) {
