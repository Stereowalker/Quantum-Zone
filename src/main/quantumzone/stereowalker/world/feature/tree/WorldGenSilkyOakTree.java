package quantumzone.stereowalker.world.feature.tree;
...
public class WorldGenSilkyOakTree extends WorldGenAbstractTree
{
    private static final IBlockState LOG = Blk.LOGS.getDefaultState().withProperty(BlockLogs.VARIANT, TreeHandler.EnumType.SILKOAK);
    private static final IBlockState LEAVES = Blk.LEAVES.getDefaultState().withProperty(BlockLeaf.VARIANT, TreeHandler.EnumType.SILKOAK).withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));

    public WorldGenSilkyOakTree()
    {
        super(false);
    }
    ...
    {
        int i = rand.nextInt(1) + rand.nextInt(3) + 5;
        int j = position.getX();
        int k = position.getY();
        int l = position.getZ();
        ...
        if (k >= 1 && k + i + 1 < 256)
        {
            ...
            boolean isSoil = state.getBlock().canSustainPlant(state, worldIn, blockpos, net.minecraft.util.EnumFacing.UP, ((BlockSaplings)Blk.SAPLING));
            ...
            else
            {
                ...
                int i1 = i - rand.nextInt(6);
                int j1 = 2 - rand.nextInt(4);
                int k1 = j;
                int l1 = l;
                int i2 = k + i - 1;
                ...
                for (int i3 = -2; i3 <= 0; ++i3)
                {
                    for (int l3 = -2; l3 <= 0; ++l3)
                    {
                        ...
                        this.placeLeafAt(worldIn, k1 + i3 + 1, i2 + k4 + 1, l1 + l3 + 1);
                        this.placeLeafAt(worldIn, 1 + k1 - i3 + 1, i2 + k4 + 1, l1 + l3 + 1);
                        this.placeLeafAt(worldIn, k1 + i3 + 1, i2 + k4 + 1, 1 + l1 - l3 + 1);
                        this.placeLeafAt(worldIn, 1 + k1 - i3 + 1, i2 + k4 + 1, 1 + l1 - l3 + 1);
                ...
                if (rand.nextBoolean())
                {
                    ...
                    this.placeLeafAt(worldIn, k1 + 1, i2 + 2 + 1, l1 + 1);
                    this.placeLeafAt(worldIn, k1 + 1 + 1, i2 + 2 + 1, l1 + 1);
                    this.placeLeafAt(worldIn, k1 + 1 + 1, i2 + 2 + 1, l1 + 1 + 1);
                    this.placeLeafAt(worldIn, k1 + 1, i2 + 2 + 1, l1 + 1 + 1);
                }
                ...
                for (int j3 = -3; j3 <= 4; ++j3)
                {
                    for (int i4 = -3; i4 <= 4; ++i4)
                    {
                        if ((j3 != -3 || i4 != -3) && (j3 != -3 || i4 != 4) && (j3 != 4 || i4 != -3) && (j3 != 4 || i4 != 4) && (Math.abs(j3) < 3 || Math.abs(i4) < 3))
                        {
                            ...
                            this.placeLeafAt(worldIn, k1 + j3 + 1, i2 + 1, l1 + i4 + 1);
                        }
                    }
                }
                        ...
                        if ((k3 < 0 || k3 > 1 || j4 < 0 || j4 > 1) && rand.nextInt(3) <= 0)
                        {
                            ...
                            for (int j5 = -1; j5 <= 1; ++j5)
                            {
                                for (int l2 = -1; l2 <= 1; ++l2)
                                {
                                    ...
                                    this.placeLeafAt(worldIn, k1 + k3 + j5 + 1, i2 + 1, l1 + j4 + l2 + 1);
                                }
                            }
                            ...
                            for (int k5 = -2; k5 <= 2; ++k5)
                            {
                                for (int l5 = -2; l5 <= 2; ++l5)
                                {
                                    if (Math.abs(k5) != 2 || Math.abs(l5) != 2)
                                    {
                                        ...
                                        this.placeLeafAt(worldIn, k1 + k3 + k5 + 1, i2 - 1 + 1, l1 + j4 + l5 + 1);
                                    }
                                }
                            }
                        }
