...
public class TeleporterQuantum extends Teleporter{
	
	private final WorldServer worldServerInstance;
    private final Random random;
    private final Long2ObjectMap<Teleporter.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap(4096);
	
	public TeleporterQuantum(WorldServer worldIn) {
		super(worldIn);
            ...
            for (int j1 = -2; j1 <= 2; ++j1)
            {
                for (int k1 = -2; k1 <= 2; ++k1)
                {
                    for (int l1 = -1; l1 < 3; ++l1)
                    {
                        ...
                        int k2 = k + k1 * 0 - j1 * 1;
                        boolean flag = l1 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(i2, j2, k2), flag ? Blk.BLUE_COBBLE.getDefaultState() : Blocks.AIR.getDefaultState());
                    }
                }
            }
	...
	public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_) {
			...
			for (int l = -128; l <= 128; ++l) {
				...
				for (int i1 = -128; i1 <= 128; ++i1) {
					for (BlockPos blockpos = blockpos4.add(l, this.worldServerInstance.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos
						...
						if (this.worldServerInstance.getBlockState(blockpos).getBlock() == Blk.PORTAL) {
							while (this.worldServerInstance.getBlockState(blockpos1 = blockpos.down()).getBlock() == Blk.PORTAL) {
								blockpos = blockpos1;
							}
		...
		if (d0 >= 0.0D) {
			...
			if (this.worldServerInstance.getBlockState(((BlockPos) object).west()).getBlock() == Blk.PORTAL) {
				enumfacing = EnumFacing.NORTH;
			}

			if (this.worldServerInstance.getBlockState(((BlockPos) object).east()).getBlock() == Blk.PORTAL) {
				enumfacing = EnumFacing.SOUTH;
			}

			if (this.worldServerInstance.getBlockState(((BlockPos) object).north()).getBlock() == Blk.PORTAL) {
				enumfacing = EnumFacing.EAST;
			}

			if (this.worldServerInstance.getBlockState(((BlockPos) object).south()).getBlock() == Blk.PORTAL) {
            ...
            for (int j7 = -1; j7 <= 1; ++j7)
            {
                for (int l7 = 1; l7 < 3; ++l7)
                {
                    for (int k8 = -1; k8 < 3; ++k8)
                    {
                        ...
                        boolean flag = k8 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(k9, k10, k11), flag ? Blk.BLUE_COBBLE.getDefaultState() : Blocks.AIR.getDefaultState());
                    }
                }
            }
        }

        IBlockState iblockstate = Blk.PORTAL.getDefaultState().withProperty(BlockPortal.AXIS, l6 == 0 ? EnumFacing.Axis.Z : EnumFacing.Axis.X);
        ...
        for (int i8 = 0; i8 < 4; ++i8)
        {
            for (int l8 = 0; l8 < 4; ++l8)
            {
                for (int l9 = -1; l9 < 4; ++l9)
                {
                    ...
                    this.worldServerInstance.setBlockState(new BlockPos(l10, l11, k12), flag1 ? Blk.BLUE_COBBLE.getDefaultState() : iblockstate, 2);
                }
            }
