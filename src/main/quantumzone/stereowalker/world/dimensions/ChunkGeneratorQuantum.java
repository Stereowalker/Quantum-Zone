...
import quantumzone.stereowalker.world.gen.structures.MapGenBlueStoneBridge;

public class ChunkGeneratorQuantum implements IChunkGenerator {

	protected static final IBlockState AIR = Blocks.AIR.getDefaultState();
    protected static final IBlockState QUANTEX = Blk.QUANTEX.getDefaultState();
    protected static final IBlockState QUANTEX1 = Blk.BLOCK_QUANTEX_SOIL.getDefaultState();
    ...
    private final WorldGenQuantumTree brownMushroomFeature = new WorldGenQuantumTree();
    private final WorldGenBush redMushroomFeature = new WorldGenBush(Blocks.RED_MUSHROOM);
    //private MapGenBlueStoneBridge genNetherBridge = new MapGenBlueStoneBridge();
    ...
    public ChunkGeneratorQuantum(World worldIn, boolean p_i45637_2_, long seed)
    {
        this.world = worldIn;
        ...
        //this.genNetherBridge = (MapGenBlueStoneBridge)net.minecraftforge.event.terraingen.TerrainGen.getModdedMapGen(genNetherBridge, net.minecraftforge.event.terraingen.InitMapGenEvent.EventType.NETHER_BRIDGE);
    ...
    public void buildSurfaces(int p_185937_1_, int p_185937_2_, ChunkPrimer primer)
    {
        ...
        for (int j = 0; j < 16; ++j)
        {
            for (int k = 0; k < 16; ++k)
            {
                ...
                int i1 = -1;
                IBlockState iblockstate = QUANTEX;
                IBlockState iblockstate1 = QUANTEX1;
                ...
                for (int j1 = 127; j1 >= 0; --j1)
                {
                    if (j1 < 127 - this.rand.nextInt(5) && j1 > this.rand.nextInt(5))
                    {
                        ...
                        {
                            if (iblockstate2.getBlock() == Blk.QUANTEX)
                            {
                                if (i1 == -1)
                                {
                                    if (l <= 0)
                                    {
                                        iblockstate = AIR;
                                        iblockstate1 = QUANTEX;
                                    }
                                    else if (j1 >= i - 4 && j1 <= i + 1)
                                    {
                                        iblockstate = QUANTEX;
                                        iblockstate1 = QUANTEX1;
    ...
    public List<Biome.SpawnListEntry> getPossibleCreatures(EnumCreatureType creatureType, BlockPos pos)
    {
        if (creatureType == EnumCreatureType.MONSTER)
        {
            ...
            if (this.genNetherBridge.isPositionInStructure(this.world, pos) && this.world.getBlockState(pos.down()).getBlock() == Blocks.COBBLESTONE)
            {
                return this.genNetherBridge.getSpawnList();
            }*/
        }
    ...
    {
        return null;//"Fort".equals(structureName) && this.genNetherBridge != null ? this.genNetherBridge.getNearestStructurePos(worldIn, position, findUnexplored) : null;
    }

    public boolean isInsideStructure(World worldIn, String structureName, BlockPos pos)
    {
        return false;//"Fort".equals(structureName) && this.genNetherBridge != null ? this.genNetherBridge.isInsideStructure(pos) : false;
    }
